<?php
	...
	function loadDB(){
		$json_db = file_get_contents("DataBase.json");
	...
	function saveDB($db){
		...
		file_put_contents("DataBase.json", $json_db);
	} // saveDB
			...
			if ($word == 'meta'){	// Meta tag was found
				...
				while ($word){		// Parse the meta tag.
					...
					if ($word == 'name'){							// Meta tag name found.
						...
						if ($word == 'keywords'){					// Meta containing keywords is ignored.
							$name = '';
						} else {
							$name = $word;
						}
	...
	// Parses the html main div of a document to get the data.
	function parseMain(&$db, &$lines, $filename){
	
		$separators = " ,\n\t\"\\\0\r/<>;=!().:&*|@#\$%^~`?";	// Set in file separators.
		$validWordCount = 0;									// Counts the number of words in the document.
		$totalWordCount = 0;									// Used to determine word location in the file.

		// Parse the main div.
		foreach ($lines as $line_num => $line){
		
			$word = strtolower(strtok($line, $separators));	// Get the first word of each line.
			if ($word == 'div'){							// If the div ends, finish main parsing.
				return $validWordCount;
			}
			
			while ($word && ($word != 'br') && ($word != 'p')){	// Parse the line's content.
			
				if (!($db{'STOPLIST'}{$word} == 1)){	// If the word is in the stop list, get the next word and go to the next iteration.
					++$validWordCount;					// Add to count only non-stop-list word.
				}
				++$totalWordCount;

				$db{'WORDS'}{$word}{$filename}{$totalWordCount} = 1;	// Add word to DB.
	...
	function addDocToDB(&$db, $filename){
		...
		if (isset($db{'FILES'}{$filename}{'ACTIVE'})){	// If the file was ever in the database, set it as active without parsing.
		...
		foreach ($lines as $line_num => $line){				// HTML head ended, look for the main div.
			$word = strtolower(strtok($line, $separators));	// Get the first word of each line.
			if ($word == 'div'){							// If the div starts, go to main parsing.
				array_shift($lines);						// Remove the line with the div tag.
				$wordCount = parseMain($db, $lines, $filename);
				$db{'FILES'}{$filename}{'WCOUNT'} = $wordCount;	// Set the total word count.
				break;
			}
			array_shift($lines);	// Remove line after parsing it.
		}
	...
	function deleteDocFromDB(&$db, $filename){
		...
		$words = array_keys($db{'WORDS'});
		foreach ($words as $word){					// For each word, remove all appearances in the file from the database.
			unset($db{'WORDS'}{$word}{$filename});
			if ($db{'WORDS'}{$word} == null){		// If this was the only document the word appeared in, delete the word as well.
				unset($db{'WORDS'}{$word});
			}
		}
	...
	function find($db, $queryStr){
		...
		foreach (array_unique($words) as $word){	// Search for each word separately and get all the documents it appears in.
			...
			foreach ($appearsInDocuments as $document){		// Check if the word appears in the metadata of the document.
				...
				++$documents{$key};		// Add this document to the results' list. Metadata words have higher value than regular words.
			}
			
			$appearsInDocuments = array_keys($db{'WORDS'}{$word});
			foreach ($appearsInDocuments as $document){		// Calculate value for each document.
				if (!$db{'FILES'}{$document}{'ACTIVE'}){	// If the document is inactive, go to the next document.
					continue;
				}
				$count = count($db{'WORDS'}{$word}{$document});
				$percentage = ($count / $db{'FILES'}{$document}{'WCOUNT'}) * 100;	// Calculate this word's percentage in the document.
				$key = $db{'FILES'}{$document}{'KEY'};
				$documents{$key} += $percentage;		// Add this document to the results' list. Multiple words increase the document's value.
			}
		}

		arsort($documents);	// Sort the documents by relevance.
	...
	function findExact($db, $queryStr){
		...
		$appearsInDocuments = array_keys($db{'WORDS'}{$firstWord});	// Find all the documents containing the first word.
		foreach ($appearsInDocuments as $document){					// Look for the phrase in each document.
			...
			$firstWordLocations = array_keys($db{'WORDS'}{$firstWord}{$document});
			foreach ($firstWordLocations as $location){		// Wherever the first word is found, go through the following words and check if they match the phrase.
				for ($i = 0; $i <= $numWordsLeft; ++$i){
					$word = $words[$i];
					$wordLocation = $location + $i + 1;
					if ($db{'WORDS'}{$word}{$document}{$wordLocation} != 1){
						break;
					}
				}
				if ($i == $numWordsLeft){	// If the phrase was found, add the document to the results' list. Multiple appearances make the document more relevant.
					$key = $db{'FILES'}{$document}{'KEY'};
					$documents{$key} += 1;
				}
			}
	...
	function searchDB($db, $queryStr){
		...
		$keys = array_keys($afterCalcQuery);	// Create a list of documents (Sorted by relevance).
		foreach ($keys as $key){
